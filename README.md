...
The tab includes a **period-by-period table** (Google / Meta / blended Total, or a single column set for one channel — toggle via the "Series" control), matching the "KPIs by Month" layout the agency's own report uses. Capped at 60 rows — pick a coarser granularity or shorter range if you'd hit more (e.g. don't ask for daily rows over a 12-month span).

### A Meta gotcha this already ran into

Meta's Insights API reports purchases under **several overlapping `action_type` values at once** — `omni_purchase` (the cross-device figure Ads Manager shows by default), `offsite_conversion.fb_pixel_purchase`, `purchase`, and others. They aren't additive; they're different views of the same sale. The first version of this fetch summed every `action_type` containing "purchase", which double/triple-counted — conversions and conversion value came out ~4-6x too high (spend was unaffected, since it's a single plain number, not an overlapping array — that mismatched pattern, spend close but conversions wildly off, is the tell if this regresses). Fixed by picking one canonical type (`omni_purchase`, falling back to `offsite_conversion.fb_pixel_purchase` then `purchase`) instead of summing. If the numbers on this tab ever stop matching the agency report again, check `extractPurchaseMetrics()` in `scripts/fetch-ads-data.js`/`backfill-ads-data.js` first — and re-run the backfill with `--force` after any fix, since the wrong figures are sitting in `ads-data.json` until then.
